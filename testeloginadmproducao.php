    ...
    if (isset($entrar)) {
      ...
      $verifica = mysql_query("SELECT * FROM usuariosadm WHERE login = '$login' AND senha = '$senha'") or die("Erro ao conectar ao banco de dados!");
        if (mysql_num_rows($verifica)<=0){
          ...
          echo"<script language='javascript' type='text/javascript'>alert('Login e/ou senha incorretos');window.location.href='loginadmproducao.html';</script>";
          die();
        }else{
          $_SESSION['login'] = $login;
		  $_SESSION['senha'] = $senha;
		  header("Location:opcaoadmproducao.php");
        }
    }
?>